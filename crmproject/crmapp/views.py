...
def listings_employee(request):
    if request.method == 'GET':
        ...
        listings_count = Listing.objects.filter(
            responsible=employee).count()  # Получаем количество объявлений для этого сотрудника
        ...
        not_agree_status = Listing.objects.filter(responsible=employee,
                                                  call_status=Listing.MISSED).count()  # Получаем количество статусов звонка "Не договорился" для этого сотрудника
        not_phone_status = Listing.objects.filter(responsible=employee,
                                                  call_status=Listing.NOT_REACHED).count()  # Получаем количество статусов звонка "Не дозвонился" для этого сотрудника
        ...
        if 'new_obect_data' in request.POST:
            data = json.loads(request.POST.get('new_obect_data'))
            listing_id = data.get('listing_id')
            if listing_id:
                try:
                    listing = Listing.objects.get(id=listing_id)
                    OurListings.objects.create(listing=listing)
                    listing.call_status = Listing.OUR_OBJECT
                    listing.save()
                    return JsonResponse({'status': 'success'})
                except Listing.DoesNotExist:
                    return JsonResponse({'status': 'error', 'message': 'Listing not found'})
            else:
                return JsonResponse({'status': 'error', 'message': 'Listing ID not provided'})
        elif 'not_agreed_data' in request.POST:
            data = json.loads(request.POST.get('not_agreed_data'))
            listing_id = data.get('listing_id')
            if listing_id:
                try:
                    listing = Listing.objects.get(id=listing_id)
                    listing.call_status = Listing.MISSED  # Статус звонка "Не договорился" ("Не снял")
                    listing.save()
                    return JsonResponse({'status': 'success'})
                except Listing.DoesNotExist:
                    return JsonResponse({'status': 'error', 'message': 'Listing not found'})
